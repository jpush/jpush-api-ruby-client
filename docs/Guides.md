...
| category | 是 | 此字段由于厂商为必填字段，效果也完全依赖 rom 厂商对 category 的处理策略，请开发者务必填写。极光内部对此字段实际未进行必填校验，请开发者按照必填处理。  此字段值对应官方「云端category」取值，开发者通过极光服务发起推送时如果传递了此字段值，请务必按照官方要求传递，官方category分类取值规则也可参考鸿蒙消息分类标准(https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/push-apply-right-V5)  |
